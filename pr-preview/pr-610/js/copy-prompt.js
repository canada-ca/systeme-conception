document.addEventListener("click", function(e) {
  ...
  navigator.clipboard.writeText(target.textContent).then(() => {
    ...
    if (status) status.textContent = "Invite copiée dans le presse-papiers.";
  }).catch(() => {
    if (status) status.textContent = "Impossible de copier l'invite. Veuillez la copier manuellement.";
  });
});
...
function resetCopiedButtons(except) {
  document.querySelectorAll(".copy-btn.copied").forEach((btn) => {
    ...
    btn.textContent = btn.dataset.originalLabel || "Copier l'invite";
