...
function responsivePreviewHandler( event, data ) {
	...
	// Validation
	if ( !$container.length ) {
		console.log( "Avertissement", componentName + " configuré sur " + $elm.get( 0 ) + " mais le sélecteur de conteneur '" + data.container + "' n'a pas été trouvé." );
		return;
	}
	...
	if ( buttonKeys.length === 0 ) {
		console.log( "Avertissement", componentName + " configuré sur " + $elm.get( 0 ) + " mais aucun sélecteur de bouton trouvé dans l'objet 'buttons' ou les propriétés de bouton héritées." );
		return;
	}
	...
	for ( i = 0; i < buttonKeys.length; i++ ) {
		...
		if ( !$button.length ) {
			console.log( "Avertissement", componentName + " sélecteur de bouton '" + buttons[key] + "' pour le point d'arrêt '" + key + "' non trouvé." );
			continue;
		}
